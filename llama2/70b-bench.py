...
tokenizer = AutoTokenizer.from_pretrained(model, cache_dir="/data")

model = AutoModelForCausalLM.from_pretrained(model, cache_dir="/data", device_map="auto", torch_dtype=torch.float16)

for nam, dev in model.hf_device_map.items():
    print(nam, dev)
